...
local fiber_stack = {}
fiber_stack.__index = fiber_stack

function fiber_stack:new()
    return setmetatable({size = 0, fibers = {}}, fiber_stack)
end

function fiber_stack:push(name, status)
...
function fiber_stack:is_empty()
    return self.size == 0
end

-- spawns a service, which invloves creation of a child context,
...
local function spawn(service, bus, ctx)
    ...
    fiber.spawn(function ()
        ...
        local active_fibers = fiber_stack:new()
