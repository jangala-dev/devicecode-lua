-- make trigger objects
-- they have an update_val and does_trigger function
-- can hold internal state
-- difftrigger can be building block for percent and absolute triggers
...
function DiffTrigger.new(diff_method, threshold, initial_val)
    if type(initial_val) ~= 'number' then return nil, "Initial value must be a number" end
    ...
    self.init_val = initial_val or 1
...
function TimeTrigger.new(duration)
