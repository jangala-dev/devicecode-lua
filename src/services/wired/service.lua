...
local function append_expected_vlan(violations, expected, observed, observed_list, surface_id, seg_id, seg, role)
	...
	if not observed[vlan_id] then
		local kind = role == 'user'
			and 'missing_user_segment_carriage'
			or 'missing_required_segment_carriage'
		append_violation(violations, kind, {
			severity = role == 'required' and 'critical' or 'error',
			surface_id = surface_id,
			segment = seg_id,
			vlan = vlan_id,
...
local function validate_protected_trunk_carriage(
	violations,
	surface_id,
	desired,
	p_ok,
...
local function rebuild_derived(snap)
	...
	for id, desired in pairs((snap.config_intent and snap.config_intent.surfaces) or {}) do
		...
		if desired.protected then
			if desired.enabled == false then
				append_violation(violations, 'protected_surface_disabled', {
					surface_id = id,
					severity = 'critical',
				})
			end
			if desired.attachment.mode ~= 'trunk' then
				append_violation(violations, 'protected_surface_not_trunk', {
					surface_id = id,
					severity = 'critical',
				})
			end
			if provider == nil then
				append_violation(violations, 'protected_provider_missing', {
					surface_id = id,
					provider_id = provider_id,
					severity = 'critical',
				})
			elseif not p_ok then
				append_violation(violations, 'protected_provider_unavailable', {
					surface_id = id,
					provider_id = provider_id,
					reason = p_reason,
					severity = 'critical',
			...
			local expected_vlans, observed_vlans = validate_protected_trunk_carriage(
				violations,
				id,
				desired,
				p_ok,
		...
		if desired.attachment.mode == 'access' then
			topology.access[id] = {
				surface_id = id,
				segment = desired.attachment.segment,
			}
		elseif desired.attachment.mode == 'trunk' then
			topology.trunks[id] = {
				surface_id = id,
				segments = copy(desired.attachment.segments),
				required_segments = copy(desired.attachment.required_segments),
				user_segments = copy(desired.attachment.user_segments),
				expanded_user_segments = expand_user_segments(desired.attachment, segments),
			}
		end
...
local function apply_config(state, ev)
	local intent, err = config_mod.normalise(ev and ev.raw or nil, {
		rev = ev and ev.rev,
		generation = ev and ev.generation,
	})
	if not intent then return nil, err end
	local _, _, uerr = state.model:update(function (snap)
...
local function apply_provider_event(state, ev)
	if ev and ev.op == 'replay_done' then return true, nil end
	local _, _, uerr = state.model:update(function (snap)
		if update_provider_from_event(snap, ev) then
			snap.stats.provider_updates = (snap.stats.provider_updates or 0) + 1
		end
		return rebuild_derived(snap)
	end)
	if uerr ~= nil then return nil, uerr end
...
function M.build_state(scope, opts)
	...
	local cfg, cfg_err = config_watch.open(conn, 'wired', {
		changed_kind = 'wired_config_changed',
		closed_kind = 'wired_config_closed',
	})
	if not cfg then return nil, cfg_err end
	local net_watch, nerr = bus_cleanup.watch_retained(conn, topics.net_segments(), {
		replay = true,
		queue_len = 8,
		full = 'reject_newest',
	})
	if not net_watch then cfg:close(); return nil, nerr end
	local provider_watch, perr = bus_cleanup.watch_retained(conn, topics.wired_provider_cap_pattern(), {
		replay = true,
		queue_len = 32,
		full = 'reject_newest',
	})
...
function M.next_event_op(state)
	return fibers.named_choice({
		...
		net = state.net_watch:recv_op():wrap(function (ev, err)
			return { kind = ev and 'net_segments_changed' or 'net_segments_closed', ev = ev, err = err }
		end),
		provider = state.provider_watch:recv_op():wrap(function (ev, err)
			return { kind = ev and 'provider_changed' or 'provider_closed', ev = ev, err = err }
		end),
