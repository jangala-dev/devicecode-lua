...
function M.cleanup_now(conn, published)
	...
	for id in pairs(published.surfaces or {}) do
		bus_cleanup.unretain(conn, projection.surface_topic(id))
		published.surfaces[id] = nil
	end
	for id in pairs(published.providers or {}) do
		bus_cleanup.unretain(conn, projection.provider_topic(id))
		published.providers[id] = nil
	end
