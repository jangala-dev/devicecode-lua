...
local function normalise_attachment(v, p)
	...
	local mode = v.mode
	if mode == nil then return nil, err({ path(p), 'mode' }, 'is required') end
...
local SURFACE_FIELDS = {
	'id', 'name', 'description', 'kind', 'role', 'enabled', 'protected',
...
local function normalise_surface(surface_id, rec, p)
	...
	if rec.attachment == nil then return nil, err({ path(p), 'attachment' }, 'is required') end
	local attachment, aerr = normalise_attachment(rec.attachment, { path(p), 'attachment' })
	...
	local name, nerr = optional_string(rec.name, { path(p), 'name' })
