...
local function id(v, p)
	...
	if not v:match(ID_PATTERN) then
		return nil, err(
			p,
			'must contain only letters, digits, underscore, hyphen or dot, and must start with a word character'
		)
	end
	return v, nil
end
...
local function normalise_surface(surface_id, rec, p)
	...
	if protected then
		...
		if #(attachment.required_segments or {}) == 0 then
			return nil, err(
				{ path(p), 'attachment', 'required_segments' },
				'protected trunks must declare required system segments'
			)
		end
	end
...
local function normalise_surfaces(v)
	if v == nil then return {}, nil end
	if not is_plain_table(v) or tablex.is_array(v) then
		return nil, err({ 'wired', 'surfaces' }, 'must be a map keyed by surface id')
	end
	local out = {}
...
local TOP_LEVEL = {
	'schema', 'version', 'product', 'description', 'surfaces',
	'policies', 'runtime', 'metadata', 'extensions',
}
...
function M.normalise(raw, opts)
	...
	if type(version) ~= 'number' or version % 1 ~= 0 or version < 1 then
		return nil, 'cfg/wired.version must be a positive integer'
	end
