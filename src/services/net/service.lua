...
local function poll_counter_metrics_once(state)
...
function M.run(scope, params)
	...
	if state.counter_poll_enabled and conn then
		scope:spawn(function () counter_metrics_loop(state) end)
	end
