...
local function install_request_source(scope, checked, registry, opts)
	...
	local pipe, perr = request_body.new_pipe({
		max_buffered_chunks = opts.request_body_buffer_chunks or opts.max_request_body_buffer_chunks or 8,
	...
	local spawned, spawn_err = scope:spawn(function ()
		local copied, cerr = fibers.perform(body.copy_source_to_sink_op(source, pipe, {
			max_bytes = opts.max_request_body_bytes or opts.maximum_request_body_bytes or math.huge,
			max_chunk = opts.max_request_body_chunk or opts.max_chunk or 65536,
		...
		return unwrap(fibers.run_scope_op(function (scope)
			...
			if sink then
				local written, werr = fibers.perform(body.copy_response_to_sink_op(ex, sink, {
					max_bytes = opts.max_response_body_bytes or opts.maximum_response_body_bytes or math.huge,
					max_chunk = opts.max_chunk or 65536,
				}))
