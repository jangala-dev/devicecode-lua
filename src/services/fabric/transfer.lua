...
local function incoming_commit(self, msg)
    ...
    if st.bytes_done ~= msg.size then
        ...
        self._send_frame(protocol.xfer_done(st.id, false, nil, 'size_mismatch'))
        finalise_in(self, st, 'aborted', 'size_mismatch')
        return true, nil
    end
    ...
    if type(st.sink.sha256hex) == 'function' then
        ...
        if type(got) ~= 'string' or got:lower() ~= tostring(msg.sha256):lower() then
            ...
            self._send_frame(protocol.xfer_done(st.id, false, nil, 'sha256_mismatch'))
            finalise_in(self, st, 'aborted', 'sha256_mismatch')
            return true, nil
        end
    end
    ...
    if info == nil and err ~= nil then
        self._send_frame(protocol.xfer_done(st.id, false, nil, tostring(err)))
        finalise_in(self, st, 'aborted', err)
        return true, nil
    end

    self._send_frame(protocol.xfer_done(st.id, true, info, nil))
    finalise_in(self, st, 'done', nil, info)
    return true, nil
end
...
function Transfer:status(id)
    ...
    if self._out and self._out.id == id then return self._history[id], nil end
    if self._in and self._in.id == id then return self._history[id], nil end
    if self._history[id] then return self._history[id], nil end
...
function Transfer:abort_all(reason)
    if self._out then self:abort(self._out.id, reason or 'aborted') end
    if self._in then self:abort(self._in.id, reason or 'aborted') end
    return true
end
