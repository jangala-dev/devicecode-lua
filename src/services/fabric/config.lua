...
--
-- Config normalisation for cfg/fabric.
--
-- Dev-branch shape:
--   links.<id>.transport.kind = "uart"
--   links.<id>.transport.cap_id = <uart capability id>
...
local function norm_transport(t, path)
    ...
    if kind == 'uart' then
        if type(t.cap_id) ~= 'string' or t.cap_id == '' then
            return nil, path .. '.cap_id must be a non-empty string'
        end
        return {
            kind           = 'uart',
            cap_id         = t.cap_id,
            open_timeout_s = (type(t.open_timeout_s) == 'number' and t.open_timeout_s > 0)
                and t.open_timeout_s
                or 30.0,
