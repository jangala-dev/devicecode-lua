...
--   * sha256_hex(bytes)
--
-- SHA-256 uses a library backend if available, falling back to a pure-Lua
...
local function sha256_lib(bytes)
	do
		local ok, openssl = pcall(require, 'openssl.digest')
		if ok and type(openssl) == 'table' and type(openssl.digest) == 'function' then
			local d = openssl.digest('sha256', bytes)
			if type(d) == 'string' then
				return (d:gsub('.', function(ch) return string.format('%02x', ch:byte()) end))
			end
		end
	end
	...
	do
		...
		if ok and type(openssl) == 'table' and openssl.digest and type(openssl.digest.new) == 'function' then
	...
	do
		...
		if ok and type(sha2) == 'table' and type(sha2.sha256hex) == 'function' then
			return sha2.sha256hex(bytes)
		end
		if ok and type(sha2) == 'table' and type(sha2.sha256) == 'function' then
			local out = sha2.sha256(bytes)
...
function M.sha256_hex(bytes)
	...
	return sha256_lib(bytes) or sha256_pure(bytes)
end
