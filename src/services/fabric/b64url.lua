...
function M.encode(s)
	...
	for i = 1, #s do
		local b = s:byte(i)
		local bb = to_bits(b, 8)
		for j = 1, #bb do bits[#bits + 1] = bb[j] end
	end
	...
	local out = {}
	for i = 1, #bits, 6 do
		out[#out + 1] = ENC:sub(from_bits(bits, i, 6) + 1, from_bits(bits, i, 6) + 1)
	end
...
function M.decode(s)
	...
	elseif rem == 1 then return nil, 'invalid_base64url_length' end
	...
	for i = 1, #s do
		...
		if ch ~= '=' then
			local v = DEC[ch]
			if v == nil then return nil, 'invalid_base64url_character' end
