...
local function normalise_policy(rec)
	local p = table_or_empty(rec and (rec.availability or rec.availability_policy))
	return {
...
local function health_from_fact(raw_health)
	...
	if type(raw_health) == 'string' then
		...
		if h == 'ok' or h == 'healthy' or h == 'ready' then return 'ok', nil end
		if h == 'warning' or h == 'warn' or h == 'degraded' then return 'warning', 'health_warning' end
		if h == 'fault' or h == 'faulted' or h == 'error' or h == 'failed' then return 'fault', 'health_fault' end
		return raw_health, nil
	end
	if type(raw_health) == 'table' then
		local h = first_non_nil(raw_health.health, raw_health.state, raw_health.status, raw_health.kind)
		local fault = first_non_nil(raw_health.fault, raw_health.fault_code, raw_health.error, raw_health.err)
		if fault ~= nil and fault ~= false then return 'fault', 'health_fault' end
		if h ~= nil then return health_from_fact(tostring(h)) end
	end
	return nil, nil
end
...
function M.component_status(rec)
	...
	if type(mod) == 'table' and type(mod.availability) == 'function' then
		...
		elseif not ok then
			return status('degraded', 'warning', 'availability_policy_failed', { error = tostring(refined) })
		end
	end
