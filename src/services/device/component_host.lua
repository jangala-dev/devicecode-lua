...
function M.normalize_fact(fact, raw)
	if fact == 'software' then return M.normalize_software(raw), nil end
	if fact == 'updater' then return M.normalize_updater(raw), nil end
	if fact == 'health' then return M.normalize_health(raw), nil end
	return copy(raw), nil
end
M.normalise_fact = M.normalize_fact

function M.normalize_event(_, raw)
	return copy(raw), nil
end
