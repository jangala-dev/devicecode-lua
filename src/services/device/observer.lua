...
local function normalise_with_component(rec, kind, name, payload)
	...
	if type(mod) == 'table' then
		if kind == 'fact' then
			fn = mod.normalise_fact
		else
			fn = mod.normalise_event
		end
	end
...
function M.run(scope, ctx)
	...
	local component_id = assert(ctx.component_id or rec.id, 'device observer requires component_id')
