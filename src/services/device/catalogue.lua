		...
		elseif type(spec) == 'table' then
			...
			if kind == 'rpc' then
				...
				out[public_name] = {
					name = public_name,
					kind = 'rpc',
					call_topic = assert_topic(topic, where .. ': action ' .. action_name .. ' call_topic'),
			...
			error(where .. ': action ' .. action_name .. ' must be a topic or table', 0)
		end
	end
...
local function normalise_component(id, spec)
	...
	local observe_opts = spec.observe_opts or spec.provider_opts
	...
	return {
		...
		availability = copy_value(spec.availability or spec.availability_policy or {}),
		facts = facts,
		events = events,
		actions = normalise_actions(spec.actions or spec.operations, where),
		raw_facts = raw_facts,
...
local function components_from_config(raw)
	...
	local list = raw.components or raw.device_components or raw.devices
	if list == nil then
