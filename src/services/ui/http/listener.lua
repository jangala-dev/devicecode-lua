...
local function emit(opts, ev)
	...
	-- Compatibility seam for older isolated listener tests. UI service code passes
	-- an event port, not a raw mailbox callback/sink.
	local done_tx = opts and opts.done_tx
	if done_tx then
		queue.assert_admit_required(done_tx, ev, 'http_listener_report_failed', 3)
	end
	return true, nil
end
...
local function obtain_listener_op(opts)
	if opts.listener or opts.server then
		return fibers.always(opts.listener or opts.server, nil)
	end
	...
	local cap_id = opts.http_cap_id or opts.cap_id or 'main'
...
function M.run(scope, opts)
	...
	local listener = opts.listener or opts.server
