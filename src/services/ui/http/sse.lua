...
function M.run(scope, owner, route, opts)
	...
	local pattern = route.pattern or opts.sse_pattern or (opts.sse and opts.sse.pattern) or { '#' }
	local watch, err = watch_owner:watch_open(pattern, {
		replay = opts.sse_replay ~= false,
		queue_len = opts.sse_queue_len or (opts.sse and opts.sse.queue_len) or 32,
		max_replay = opts.sse_max_replay or (opts.sse and opts.sse.max_replay),
	})
	if not watch then
