...
local function copy_plain(x, seen)
	...
	if getmetatable(x) ~= nil then error('copy_plain: metatables are not supported', 2) end
	seen = seen or {}
	if seen[x] then return seen[x] end
	local out = {}
	seen[x] = out
	for k, v in pairs(x) do out[copy_plain(k, seen)] = copy_plain(v, seen) end
	return out
end
...
local function normalise_topic(topic, opts)
	...
	if not n then return nil, 'topic must be a dense array' end
	...
	for i = 1, n do
		...
		if tv ~= 'string' and not (allow_numbers and tv == 'number') then
			return nil, ('topic[%d] must be %s'):format(i, allow_numbers and 'a string or number' or 'a string')
		end
		if tv == 'string' then
			if v == '' then
...
local function topic_debug(topic)
	...
	local parts = {}
	for i = 1, n do parts[i] = tostring(topic[i]) end
	return table.concat(parts, '/')
end
