...
local function fd_ready_op(fd, want)
	...
	return wait.waitable(register, step, function(ok) return ok end)
end
...
function M.install()
	...
	old_step = cqueues.interpose('step', function(self, timeout)
		...
		if timeout ~= nil then t = math.min(t, timeout) end
		local events = self:events()
		local fd = self:pollfd()
		local choices = {}

		if fd ~= nil and events ~= nil then
			if events ~= 'w' then choices[#choices + 1] = fd_ready_op(fd, 'rd') end
			if events ~= 'r' then choices[#choices + 1] = fd_ready_op(fd, 'wr') end
		end
		if t ~= math.huge then choices[#choices + 1] = sleep.sleep_op(t) end
