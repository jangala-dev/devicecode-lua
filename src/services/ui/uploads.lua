...
function Uploads:upload_update(session_id, stream, req_headers)
    ...
    local out, cerr = self._with_user_conn(rec.principal, { ui = { op = 'update_upload', component = component } }, function(user_conn)
        ...
        while true do
            ...
            offset = offset + #chunk
            self:_set_session(upload_id, { state = 'receiving', sent = offset, total = content_length, updated_at = os.time() })
        end

        self:_set_session(upload_id, { state = 'committing', sent = offset, total = content_length, updated_at = os.time() })
        ...
        if created == nil then
            ...
            self:_set_session(upload_id, { state = 'failed', error = tostring(uerr or 'update_create_failed'), sent = offset, updated_at = os.time() })
        ...
        self:_set_session(upload_id, { state = 'complete', sent = offset, total = content_length, updated_at = os.time() })
