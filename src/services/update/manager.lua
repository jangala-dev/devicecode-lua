...
local function handle_get(ctx, req, payload)
	local owner = request_owner.new(req)
	local job_id = type(payload) == 'table' and (payload.job_id or payload.id) or nil
...
local function is_ingest_method(method)
	return method == 'ingest_create' or method == 'create_ingest'
		or method == 'ingest_append' or method == 'append_ingest'
		or method == 'ingest_commit' or method == 'commit_ingest'
		or method == 'ingest_abort' or method == 'abort_ingest'
end
...
local function handle_start(ctx, req, payload)
	...
	local job_id = payload.job_id or payload.id
