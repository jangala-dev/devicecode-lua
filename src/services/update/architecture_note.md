...
adapter      compatibility boundary for callback or non-Op legacy code
```
...
```text
persist job.status = "staging" and active_intent.phase = "stage"
...
```text
persist job.status = "committing" and active_intent.phase = "commit"
...
```text
persist job.status = "awaiting_return" or equivalent durable state
