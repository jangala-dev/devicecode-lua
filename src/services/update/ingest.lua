...
function M.new_instance(scope, params)
	...
	local self = setmetatable({
		ingest_id = assert(params.ingest_id or params.id, 'ingest_id required'),
...
local function create_instance_now(state, req, payload)
	...
	local ingest_id = payload.ingest_id or payload.id
...
function State:handle_event(ctx, ev)
	...
	if method == 'ingest_create' or method == 'create_ingest' then
	...
	if method == 'ingest_append' or method == 'append_ingest' then
		enqueue_instance_work(ctx, self, inst, req, method, payload, function (_, owner)
			local result, err = fibers.perform(assert(inst:append_op(payload.chunk or payload.data or '')))
			if not result then error(err or 'ingest_append_failed', 0) end
	...
	if method == 'ingest_commit' or method == 'commit_ingest' then
		enqueue_instance_work(ctx, self, inst, req, method, payload, function (_, owner)
	...
	if method == 'ingest_abort' or method == 'abort_ingest' then
		enqueue_instance_work(ctx, self, inst, req, method, payload, function (_, owner)
