...
function M.load_store(state, loaded)
    ...
    for _, id in ipairs(state.store.order) do
        local job = state.store.jobs[id]
        if job then job.runtime = type(job.runtime) == 'table' and job.runtime or {} end
    end
    M.seed_seq_from_store(state)
end

function M.mark_job_dirty(state, job_id)
    if type(job_id) == 'string' and job_id ~= '' then state.dirty_jobs[job_id] = true end
    state.summary_dirty = true
end

function M.mark_all_jobs_dirty(state)
    for _, id in ipairs(state.store.order) do state.dirty_jobs[id] = true end
    state.summary_dirty = true
end
...
function M.touch_job(state, job, now_mono, service_run_id, state_changed, runtime_merge)
    if runtime_merge then
        ...
        for k, v in pairs(runtime_merge) do job.runtime[k] = v end
    end
...
function M.remove_job(state, job_id)
    ...
    for i = #state.store.order, 1, -1 do
        if state.store.order[i] == job_id then table.remove(state.store.order, i) break end
    end
    M.mark_summary_dirty(state)
end

function M.select_resumable_job(state)
    if state.active_job or state.locks.global ~= nil then return nil end
    for _, id in ipairs(state.store.order) do
        local job = state.store.jobs[id]
...
function M.can_activate(state, job)
    if state.active_job and state.active_job.job_id == job.job_id then return nil, 'job_already_active' end
    if state.locks.global ~= nil then return nil, 'busy_global' end
    return true, nil
end
        ...
        if job then
            if job.state == 'staging' then
                if type(job.artifact_ref) == 'string' and job.artifact_ref ~= '' then
                    M.patch_job(state, job, { state = 'created', next_step = nil, error = job.error }, now_mono, service_run_id, {
                        runtime_merge = { adopted = true },
                    })
                else
                    M.patch_job(state, job, { state = 'failed', next_step = nil, error = 'interrupted_before_stage' }, now_mono, service_run_id, {
                        runtime_merge = { adopted = true },
                    })
                end
                changed = true
            elseif job.state == 'awaiting_return' then
                M.patch_job(state, job, { state = 'awaiting_return', next_step = 'reconcile' }, now_mono, service_run_id, {
                    runtime_merge = { adopted = true },
                })
                changed = true
            elseif job.state == 'awaiting_approval' or job.state == 'deferred' or job.state == 'staged' then
                M.patch_job(state, job, { state = 'awaiting_commit', next_step = 'commit' }, now_mono, service_run_id, {
                    runtime_merge = { adopted = true },
                })
                changed = true
            end
