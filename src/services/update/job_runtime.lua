...
local function transition_public(cmd)
	...
	return {
		transition = cmd.kind,
		job_id = cmd.job_id or (cmd.payload and (cmd.payload.job_id or cmd.payload.id)),
...
local function adopt_restart_jobs(self, jobs)
	...
	for _, id in ipairs(sorted_job_ids(jobs)) do
		...
		elseif state == 'staging' or state == 'committing' then
			...
			local job = copy(current)
			local seq = next_adoption_seq(jobs)
			local intent = type(job.active_intent) == 'table' and job.active_intent or { token = job.active_token, phase = job.phase }
			local phase = intent and intent.phase or job.phase
...
local function compute_create(self, cmd)
	...
	payload.job_id = payload.job_id or payload.id or cmd.job_id or new_id('job')
