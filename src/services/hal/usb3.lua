...
local function set_usb_port_auth(ctx, enabled, hub, port)
    ...
    local cmd = exec.command_context(ctx, 'echo', writeint, '>', usb_addr)
...
local function set_usb_hub_auth_default(ctx, enabled, hub)
    ...
    local cmd = exec.command_context(ctx, 'echo', writeint, '>', hub_addr)
    local out, err = cmd:combined_output()
...
local function set_usb_hub_power(ctx, enabled, hub)
    ...
    local writeint = enabled and 1 or 0
    local cmd = exec.command_context(ctx, 'uhubctl', '-e', '-l', hub, '-a', writeint)
    local out, err = cmd:combined_output()
...
local function get_vl805_version_timestamp(ctx)
    local cmd = exec.command_context(ctx, 'vcgencmd', 'bootloader_version')
    local out, err = cmd:combined_output()
    if err then
        log.error("USB3: %s %s", out, err); err = err or ""; return nil, out .. err
    end
