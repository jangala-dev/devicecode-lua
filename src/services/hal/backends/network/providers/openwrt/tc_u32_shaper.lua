...
-- Provider-owned command runner override.  The OpenWrt network provider
-- serialises shaping calls, and this module also keeps backend-local STATE, so
-- the scoped override is deliberately module-local rather than a public API.
...
local function run_cmd(argv)
	if type(ACTIVE_RUN_CMD) == 'function' then
		local ok, out, err, code, st, sig = ACTIVE_RUN_CMD(argv)
		if ok == true then
			return true, out or '', err, code or 0, st or 'exited', sig
		end
...
local function first_host_cidr(spec)
	local function scan(cfg)
		if not is_plain_table(cfg) or not is_plain_table(cfg.hosts) then return nil end
		local ks = sorted_keys(cfg.hosts)
		for i = 1, #ks do
			local ip = tostring(ks[i])
			if ip:match('^%d+%.%d+%.%d+%.%d+$') then return ip .. '/32' end
		end
		return nil
	end
	return scan(spec.egress) or scan(spec.ingress)
end

local function apply_request(req, opts)
	req = req or {}
	opts = opts or {}
	local links = req.links or req.policy
	if is_plain_table(links) then
		...
		for _, link_id in ipairs(sorted_keys(links)) do
			...
			if is_plain_table(link) then
				local one = shallow_copy(link)
				one.iface = one.iface or one.device or one.linux_interface or one.interface
				one.subnet = one.subnet or one.cidr or req.subnet or req.cidr or first_host_cidr(one)
				if type(one.iface) ~= 'string' or one.iface == '' then
					return { ok = false, err = 'shaping link ' .. tostring(link_id) .. ' missing iface/device', backend = 'openwrt' }
				end
				if type(one.subnet) ~= 'string' or one.subnet == '' then
					return { ok = false, err = 'shaping link ' .. tostring(link_id) .. ' missing subnet/cidr', backend = 'openwrt' }
				end
				local ok, err = apply_one(one)
				if ok ~= true then
					return { ok = false, err = tostring(link_id) .. ': ' .. tostring(err), backend = 'openwrt' }
				end
				applied[link_id] = { ok = true, iface = one.iface, subnet = one.subnet }
				changed = true
			end
		end
	...
	local ok, err = apply_one(req)
	if ok ~= true then return { ok = false, err = tostring(err), backend = 'openwrt' } end
	return { ok = true, applied = true, changed = true, backend = 'openwrt' }
end

function M.apply(req, opts)
	opts = opts or {}
	local previous = ACTIVE_RUN_CMD
	ACTIVE_RUN_CMD = opts.run_cmd or (req and req.run_cmd)
	local ok, result = safe.pcall(apply_request, req or {}, opts)
	ACTIVE_RUN_CMD = previous
	if not ok then return { ok = false, err = tostring(result), backend = 'openwrt' } end
	return result
end
...
function M.clear(iface, opts)
	opts = opts or {}
	local previous = ACTIVE_RUN_CMD
	ACTIVE_RUN_CMD = opts.run_cmd or opts.runner
	if type(iface) ~= 'string' or iface == '' then
		ACTIVE_RUN_CMD = previous
		return nil, 'iface is required'
	end
