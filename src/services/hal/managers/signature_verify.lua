...
local function emit_device_added_op(driver, caps)
	return device_events.added_op(S.dev_ev_ch, 'signature_verify', driver.id, { provider = 'hal.signature_verify' }, caps)
end

local function emit_device_removed_op(driver)
	return device_events.removed_op(S.dev_ev_ch, 'signature_verify', driver.id, { provider = 'hal.signature_verify' })
end
...
local function shell_loop(generation, cfg_ch)
	assert(S.scope, 'signature_verify shell without scope')
...
function M.apply_config_op(cfg)
	return fibers.run_scope_op(function ()
		...
		local admitted, admit_err = fibers.perform(cfg_ch:put_op({
			...
			return false, 'signature_verify_manager_config_busy'
		end))

		if admitted ~= true then
			return false, tostring(admit_err or 'signature_verify_manager_config_busy')
		end
...
function M.terminate(reason)
	reason = tostring(reason or 'signature_verify manager terminated')
