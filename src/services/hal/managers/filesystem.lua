...
local function emit_removed_device(namespaces)
    ...
    if not removed_event then
        FilesystemManager.logger:error({ what = 'removed_device_event_create_failed', err = tostring(removed_err) })
        return
    end
...
local function stop_driver(driver)
    FilesystemManager.logger:debug({ what = 'stopping_previous_driver' })
    local stop_ok, stop_err = driver:stop(STOP_TIMEOUT)
    if not stop_ok then
        FilesystemManager.logger:warn({ what = 'stop_previous_driver_failed', err = tostring(stop_err) })
    end
end
...
local function emit_added_device(namespaces, capabilities)
    ...
    if not device_event then
        FilesystemManager.logger:error({ what = 'added_device_event_create_failed', err = tostring(ev_err) })
        return
    end
...
function FilesystemManager.start(logger, dev_ev_ch, cap_emit_ch)
    ...
    scope:finally(function()
        ...
        if st == 'failed' then
            FilesystemManager.logger:error({ what = 'scope_failed', err = tostring(primary), status = st })
        end
        FilesystemManager.logger:debug({ what = 'stopped' })
    end)

    FilesystemManager.started = true
    FilesystemManager.logger:debug({ what = 'started' })
    return ""
end
...
function FilesystemManager.apply_config(namespaces)
    ...
    local ok, spawn_err = FilesystemManager.scope:spawn(function()
        ...
        if roots_err ~= "" then
            FilesystemManager.logger:error({ what = 'build_roots_failed', err = tostring(roots_err) })
            return
        end

        local driver, init_err = init_driver(roots)
        if not driver then
            FilesystemManager.logger:error({ what = 'driver_init_failed', err = tostring(init_err) })
            return
        end

        local capabilities, cap_err = apply_driver_capabilities(driver)
        if not capabilities then
            FilesystemManager.logger:error({ what = 'apply_capabilities_failed', err = tostring(cap_err) })
            return
        end

        local start_err = start_driver(driver)
        if start_err ~= "" then
            FilesystemManager.logger:error({ what = 'driver_start_failed', err = tostring(start_err) })
            return
        end
