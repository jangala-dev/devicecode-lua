...
--
-- End-to-end firmware update harness. Boots a local hal + config + fabric
-- runtime against mcu-dev (or the config you name), waits for the fabric link
-- to come up, pushes a firmware image through rpc/fabric/send_firmware, and
-- polls transfer_status until the MCU reports done.
--
-- Usage:
--   cd devicecode-lua
--   DEVICECODE_CONFIG_DIR=$(pwd)/src/configs \
...
--
-- Env knobs:
--   DEVICECODE_TRANSFER_TIMEOUT_S   hard ceiling on poll loop (default 900s)
--   DEVICECODE_TRANSFER_CHUNK_RAW   override runtime chunk_raw for the send
--   DEVICECODE_VERIFY_AFTER_REBOOT  if 1, reconnect the local runtime after
--                                    the transfer and wait for peer hal/dump
...
local function start_runtime(scope, bus, env)
	...
	local ok_spawn, spawn_err = scope:spawn(function()
		return mainmod.run(child, {
			env          = env,
			services_csv = 'hal,config,fabric',
			bus          = bus,
		})
	end)
...
local function main(scope)
	...
	-- The config service reads ${CONFIG_TARGET}.json from the HAL fs cap,
	-- so pin it to the harness argument so the caller does not need to
	-- set CONFIG_TARGET separately.
	assert(stdlib.setenv('CONFIG_TARGET', config_name))

	-- HAL bootstrap needs DEVICECODE_CONFIG_DIR to mount the config fs cap.
	-- If the caller did not set it, point at the checked-in src/configs.
	if not os.getenv('DEVICECODE_CONFIG_DIR') then
		assert(stdlib.setenv('DEVICECODE_CONFIG_DIR', root .. 'src/configs/'))
	end

	local source, serr = blob_source.from_file(image_path, { format = 'bin' })
	if not source then
		fatal('failed to read image: ' .. tostring(serr))
	end
	...
	local child, startup_err = start_runtime(scope, bus, env)
	if not child then
		fatal(startup_err)
	end
	...
	child, startup_err = start_runtime(scope, bus, env)
	if not child then
		fatal(startup_err)
	end
