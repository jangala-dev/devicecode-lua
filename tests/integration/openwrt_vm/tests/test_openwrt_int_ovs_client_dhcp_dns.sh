...
REMOTE="/tmp/devicecode-int-ovs-client-dhcp-dns"
WORK="$VM_DIR/work/int-ovs-client-dhcp-dns"
...
ns_exec() {
	as_root ip netns exec "$OPENWRT_OVS_CLIENT_NS" "$@"
}
...
host_has_ns() {
	ip netns list 2>/dev/null | awk '{print $1}' | grep -qx "$OPENWRT_OVS_CLIENT_NS"
}

dump_diag() {
	echo "--- host bridge/client diagnostics ---" >&2
	ip -br link show dev "$OPENWRT_OVS_TAP_IFACE" >&2 2>/dev/null || true
	ip -br link show dev "$OPENWRT_OVS_CLIENT_HOST_IFACE" >&2 2>/dev/null || true
	if host_has_ns; then
		as_root ip netns exec "$OPENWRT_OVS_CLIENT_NS" ip -br addr >&2 2>/dev/null || true
		as_root ip netns exec "$OPENWRT_OVS_CLIENT_NS" ip route >&2 2>/dev/null || true
	fi
	if ip link show dev "$OPENWRT_OVS_BRIDGE" >/dev/null 2>&1; then
		ip -d link show dev "$OPENWRT_OVS_BRIDGE" >&2 2>/dev/null || true
		bridge vlan show dev "$OPENWRT_OVS_TAP_IFACE" >&2 2>/dev/null || true
		bridge vlan show dev "$OPENWRT_OVS_CLIENT_HOST_IFACE" >&2 2>/dev/null || true
		bridge link show master "$OPENWRT_OVS_BRIDGE" >&2 2>/dev/null || true
	fi
...
host_has_ns || fail "missing host network namespace $OPENWRT_OVS_CLIENT_NS; run scripts/setup-ovs-client-fabric first"
ip link show dev "$OPENWRT_OVS_TAP_IFACE" >/dev/null 2>&1 || fail "missing tap $OPENWRT_OVS_TAP_IFACE; run VM with OPENWRT_TAP_IFACES=$OPENWRT_OVS_TAP_IFACE"
ip link show dev "$OPENWRT_OVS_BRIDGE" >/dev/null 2>&1 || fail "missing host client bridge $OPENWRT_OVS_BRIDGE"
bridge vlan show dev "$OPENWRT_OVS_TAP_IFACE" | grep -q " $OPENWRT_OVS_CLIENT_VLAN" || fail "tap $OPENWRT_OVS_TAP_IFACE is not trunking VLAN $OPENWRT_OVS_CLIENT_VLAN"
bridge vlan show dev "$OPENWRT_OVS_CLIENT_HOST_IFACE" | grep -q " $OPENWRT_OVS_CLIENT_VLAN" || fail "client host veth $OPENWRT_OVS_CLIENT_HOST_IFACE is not an access port for VLAN $OPENWRT_OVS_CLIENT_VLAN"

GUEST_TRUNK_IFACE="eth$((OPENWRT_VM_WAN_IFACES + 1))"
mkdir -p "$WORK"
cat > "$WORK/run_openwrt_int_ovs_client_config.lua" <<'LUA'
...
fibers.run(function()
  local provider = assert(provider_loader.new({
    ...
    run_cmd = function(argv)
      print('int-ovs-client config-only: skipped activation command ' .. table.concat(argv or {}, ' '))
      return true, nil
    end,
  ...
  local result = perform(provider:apply_op({ intent = intent, opts = { generation = 1002024, apply_id = 'int-ovs-client-dhcp-dns' } }))
...
"$SCP_TO" "$WORK/run_openwrt_int_ovs_client_config.lua" "$REMOTE/run_openwrt_int_ovs_client_config.lua"
"$SSH" "cd '$REMOTE' && DEVICECODE_TEST_TRUNK_IFACE='$GUEST_TRUNK_IFACE' DEVICECODE_TEST_INT_VLAN='$OPENWRT_OVS_CLIENT_VLAN' lua ./run_openwrt_int_ovs_client_config.lua"
...
wait_for_router "router public DNS" 45 "nslookup '$OPENWRT_OVS_PUBLIC_DNS_NAME' 1.1.1.1 | grep -q '^Name:'"
...
wait_for_router "int DNS public forwarding" 45 "nslookup '$OPENWRT_OVS_PUBLIC_DNS_NAME' 172.28.100.1 | grep -q '^Name:'"
...
as_root ip netns exec "$OPENWRT_OVS_CLIENT_NS" ip addr flush dev "$OPENWRT_OVS_CLIENT_NS_IFACE" || true
as_root ip netns exec "$OPENWRT_OVS_CLIENT_NS" ip route flush dev "$OPENWRT_OVS_CLIENT_NS_IFACE" || true
as_root ip netns exec "$OPENWRT_OVS_CLIENT_NS" ip addr add "${OPENWRT_OVS_CLIENT_CIDR_PREFIX}250/24" dev "$OPENWRT_OVS_CLIENT_NS_IFACE"
check_static_tmp="$WORK/static-ping.out"
if ! as_root ip netns exec "$OPENWRT_OVS_CLIENT_NS" ping -c 2 -W 2 "$OPENWRT_OVS_CLIENT_ROUTER" >"$check_static_tmp" 2>&1; then
	cat "$check_static_tmp" >&2 || true
	fail "static int client could not reach $OPENWRT_OVS_CLIENT_ROUTER before DHCP"
fi
...
LEASE_ENV="$WORK/ovs-client-lease.env"
LEASE_SCRIPT="$WORK/ovs-client-udhcpc.sh"
rm -f "$LEASE_ENV"
...
as_root ip netns exec "$OPENWRT_OVS_CLIENT_NS" sh -c "$UDHCPC_CMD -i '$OPENWRT_OVS_CLIENT_NS_IFACE' -q -n -t 10 -T 1 -s '$LEASE_SCRIPT'" || { "$SSH" 'if [ -f /tmp/dcint-dhcp-br-int.pid ]; then kill $(cat /tmp/dcint-dhcp-br-int.pid) 2>/dev/null || true; fi' || true; fail "host-side bridge client did not obtain an int lease"; }
...
as_root mkdir -p "/etc/netns/$OPENWRT_OVS_CLIENT_NS"
printf 'nameserver %s\n' "$OPENWRT_OVS_CLIENT_ROUTER" | as_root tee "/etc/netns/$OPENWRT_OVS_CLIENT_NS/resolv.conf" >/dev/null
[ -f "$LEASE_ENV" ] || fail "host-side client lease environment was not captured"
# shellcheck disable=SC1090
. "$LEASE_ENV"
case "$ip" in "$OPENWRT_OVS_CLIENT_CIDR_PREFIX"*) ;; *) fail "lease IP $ip is not in ${OPENWRT_OVS_CLIENT_CIDR_PREFIX}0/24" ;; esac
case " ${router:-} " in *" $OPENWRT_OVS_CLIENT_ROUTER "*) ;; *) fail "router option does not contain $OPENWRT_OVS_CLIENT_ROUTER: ${router:-}" ;; esac
case " ${dns:-} " in *" $OPENWRT_OVS_CLIENT_ROUTER "*) ;; *) fail "DNS option does not contain $OPENWRT_OVS_CLIENT_ROUTER: ${dns:-}" ;; esac
...
check_public_dns() {
	...
	deadline=$(( $(date +%s) + 45 ))
	while [ "$(date +%s)" -le "$deadline" ]; do
		if ns_exec nslookup "$OPENWRT_OVS_PUBLIC_DNS_NAME" "$OPENWRT_OVS_CLIENT_ROUTER" >"$tmp" 2>&1 && grep -q '^Name:' "$tmp"; then
			rm -f "$tmp"
			return 0
		fi
		sleep 1
	...
	rm -f "$tmp"
	fail "$label could not resolve $OPENWRT_OVS_PUBLIC_DNS_NAME via $OPENWRT_OVS_CLIENT_ROUTER"
}

check_ns "client can ping int router" 20 ping -c 2 -W 2 "$OPENWRT_OVS_CLIENT_ROUTER"
...
ns_exec nslookup config.bigbox.home "$OPENWRT_OVS_CLIENT_ROUTER" | grep -q '192.168.1.1' || fail "client could not resolve config.bigbox.home via int DNS"
check_public_dns "client public DNS"

if command -v wget >/dev/null 2>&1; then
	check_ns "client public HTTP fetch" 45 wget -q -T 10 -O /dev/null "$OPENWRT_OVS_PUBLIC_WEB_URL"
elif command -v curl >/dev/null 2>&1; then
	check_ns "client public HTTP fetch" 45 curl -fsSL --max-time 15 -o /dev/null "$OPENWRT_OVS_PUBLIC_WEB_URL"
else
