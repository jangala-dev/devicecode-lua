-- tests/integration/devhost/ui_stack_spec.lua
...
function T.devhost_ui_controls_real_stack_via_login_config_and_rpc()
	runfibers.run(function(scope)
		...
		local svc_status, serr = ui_api.api.service_status(login.session_id, 'net')
		assert(svc_status ~= nil, tostring(serr))
		assert(type(svc_status) == 'table')
		assert(type(svc_status.state) == 'string')
