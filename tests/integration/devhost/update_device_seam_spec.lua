...
function T.devhost_update_uses_device_seam_even_when_member_topics_are_remapped()
	runfibers.run(function(scope)
			...
			components = {
				mcu = {
					...
					transfer = {
						...
						receiver = { 'rpc', 'weird', 'mcu', 'receive' },
						timeout_s = 1.0,
					},
				},
			},
		...
		local function publish_remote_status()
			remote_member_conn:retain({ 'selfish', 'software' }, { version = versions.mcu, boot_id = boot_id.mcu })
			remote_member_conn:retain({ 'selfish', 'updater' }, { state = 'running' })
			remote_member_conn:retain({ 'selfish', 'health' }, { state = 'ok' })
		end
		...
		bind_reply_loop(scope, prepare_ep, function(_payload) return { ok = true, prepared = true } end)
		bind_reply_loop(scope, receive_ep, function(_payload) return { ok = true, accepted = true } end)
		...
		local ok1, err1 = scope:spawn(function()
			session.run({
				...
				cfg = {
					...
					outbound_call_rules = {
						{ ['local'] = { 'rpc', 'weird', 'mcu' }, ['remote'] = { 'rpc', 'member', 'mcu' }, timeout = 1.0 },
					},
				},
			})
		end)
		...
		local ok3, err3 = scope:spawn(function() device.start(bus:connect(), { name = 'device', env = 'dev' }) end)
		assert(ok3, tostring(err3))
		local ok4, err4 = scope:spawn(function() update.start(bus:connect(), { name = 'update', env = 'dev' }) end)
		...
		assert(wait_device_component(caller, 'mcu', function(payload)
			return payload.available == true and payload.software.version == 'mcu-v0'
		end, 1.5))
		...
		assert(wait_retained_state(caller, { 'state', 'update', 'jobs', job.job_id }, function(payload)
			return type(payload) == 'table' and type(payload.job) == 'table' and payload.job.lifecycle.state == 'succeeded'
		end, 2.0))
		assert(wait_device_component(caller, 'mcu', function(payload)
			return payload.available == true and payload.software.version == 'mcu-v1' and payload.software.boot_id == 'mcu-boot-2'
		end, 2.0))
	end, { timeout = 4.0 })
end
