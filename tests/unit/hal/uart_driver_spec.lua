...
function T.start_op_emits_initial_meta_and_status()
	...
	runfibers.run(function(scope)
		...
		local ok_stop, err_stop = fibers.perform(d:shutdown_op())
...
function T.session_terminate_releases_active_driver_lease()
	local uart = fresh_driver()
	...
	runfibers.run(function(scope)
		...
		local ok_term, term_err = reply.session:terminate('test terminate')
		assert(ok_term == true, tostring(term_err))
		assert(d.active_session == nil)
		assert(d.active_lease_id == nil)

		local ok2, reply2 = fibers.perform(call_control_op(d.control_ch, 'open', opts))
		assert(ok2 == true, tostring(reply2))
		wait_emit(emit_ch, 'uart', 'mcu', 'state', 'status', 1.0)
		wait_emit(emit_ch, 'uart', 'mcu', 'event', 'opened', 1.0)
...
function T.shutdown_op_closes_active_session_best_effort()
	local uart = fresh_driver()
