...
function T.start_op_and_shutdown_op_round_trip()
...
function T.apply_config_op_accepts_serial_ports_wrapper_from_hal_config()
	local M = fresh_manager()
...
function T.apply_config_op_rejects_invalid_wrapped_config()
	local M = fresh_manager()
	...
	runfibers.run(function()
		...
		assert(ok_cfg == false)
		assert(tostring(err_cfg):match('serial_ports must be a list'))
...
function T.apply_config_op_rejects_non_list_table_without_serial_ports()
	local M = fresh_manager()
	...
	runfibers.run(function()
		...
		local ok_cfg, err_cfg = fibers.perform(M.apply_config_op({
			ports = {
				{ id = 'uart0', path = '/dev/ttyS0' },
			},
		}))
		assert(ok_cfg == false)
		assert(tostring(err_cfg):match('serial_ports list'))
