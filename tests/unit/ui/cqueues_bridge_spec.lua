-- tests/unit/ui/cqueues_bridge_spec.lua
...
function T.bridge_external_step_builds_wait_choices_and_calls_old_step_zero()
	...
	local perform_calls = {}
	local choice_calls = {}
	local waitable_calls = {}
	local sleep_calls = {}
	local poller_get_calls = 0
	...
	with_stubbed_modules({
		...
		['fibers.wait'] = {
			waitable = function(register, step, wrap)
				local ev = {
					tag = 'waitable',
					register = register,
					step = step,
				...
				waitable_calls[#waitable_calls + 1] = ev
				return ev
			end,
		},
		['fibers.runtime'] = {
		...
		['fibers.io.poller'] = {
			get = function()
				poller_get_calls = poller_get_calls + 1
				return {
...
function T.bridge_running_inside_fiber_yields_then_calls_old_step_with_original_timeout()
	...
	with_stubbed_modules({
		...
		['fibers.wait'] = {
			waitable = function(_, _, _) return { tag = 'waitable' } end,
		},
		['fibers.runtime'] = {
