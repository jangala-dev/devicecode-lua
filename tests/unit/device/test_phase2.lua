	...
	return {
		...
		components = {
			mcu = {
				...
				actions = { restart = topics.raw_member_cap_rpc('mcu', 'control', 'main', 'restart') },
			},
		},
	}
end
...
function tests.test_fabric_stage_consumed_source_is_not_terminated_by_action_scope()
...
function tests.test_fabric_stage_consumed_source_requires_receiver_termination_proof()
	local terminate_count = 0
	fibers.run(function (scope)
		...
		local client = {
			send_blob_op = function ()
				return op.always({ ok = true, staged = true, consumed_source = true })
			end,
		}
...
function tests.test_fabric_stage_open_source_may_be_an_explicit_op()
	fibers.run(function (scope)
		...
		local client = {
			send_blob_op = function (_, src)
				assert_eq(src, source)
				return op.always({ ok = true, staged = true, consumed_source = true, receiver_owns_source = true })
			end,
		}
...
function tests.test_fabric_stage_timeout_cancels_child_stage_and_terminates_unhanded_source()
	...
	fibers.run(function ()
		...
		local client = {
			send_blob_op = function ()
				return sleep.sleep_op(60):wrap(function ()
					return { ok = true, consumed_source = true }
				end)
			end,
		}
	...
	fibers.run(function (scope)
		...
		local client = {
			send_blob_op = function ()
				entered_stage:signal()
				return sleep.sleep_op(60):wrap(function ()
					return { ok = true, consumed_source = true, receiver_owns_source = true }
				end)
			end,
		}
...
function tests.test_mcu_fault_availability_is_stored_and_projected()
	fibers.run(function ()
		...
		m:apply_observation(1, { component = 'mcu', tag = 'fact_retained', fact = 'health', payload = { fault_code = 'brownout' } })
...
function tests.test_component_module_normalises_mcu_software_fact()
	local v, err = component_mcu.normalize_fact('software', { fw_version = '2.3', build_id = 'abc' })
	assert_nil(err)
