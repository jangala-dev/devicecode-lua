...
function tests.test_component_software_projection_recovers_after_raw_unretain_and_retain()
	local cat = assert(config.to_catalogue({
		schema = config.SCHEMA,
		components = {
			mcu = { subtype = 'mcu', facts = { software = topics.raw_member_state('mcu', 'software') } },
	...
	local m = model_mod.new()
	m:apply_catalogue(1, cat)
	m:apply_observation(1, { component = 'mcu', tag = 'fact_retained', fact = 'software', payload = { image_id = 'old', boot_id = 'boot-old' } })
	m:apply_observation(1, { component = 'mcu', tag = 'fact_unretained', fact = 'software' })

	local conn = fake_conn()
	local ok, err = publisher.publish_component_now(conn, m:snapshot(), 'mcu', { now = function () return 123 end })
	assert_true(ok, err)
	assert_nil(conn.retained['state/device/component/mcu/software'].boot_id)

	m:apply_observation(1, { component = 'mcu', tag = 'fact_retained', fact = 'software', payload = { image_id = 'new', version = '15.5', boot_id = 'boot-new' } })
	ok, err = publisher.publish_component_now(conn, m:snapshot(), 'mcu', { now = function () return 124 end })
	assert_true(ok, err)
	assert_eq(conn.retained['state/device/component/mcu/software'].image_id, 'new')
	assert_eq(conn.retained['state/device/component/mcu/software'].boot_id, 'boot-new')
end
