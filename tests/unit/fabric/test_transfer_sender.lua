...
function tests.test_sender_sends_begin_chunks_commit_and_returns_after_done()
	...
	assert_eq(progress[1].chunk_size, 3)
	assert_eq(progress[1].last_transfer_event, 'begin_tx')
	assert_eq(progress[2].status, 'sending')
	assert_eq(progress[2].sent, 0)
	assert_eq(progress[2].chunk_size, 3)
	assert_eq(progress[2].pending_offset, 0)
	assert_eq(progress[2].pending_next, 3)
	assert_eq(progress[2].last_transfer_event, 'chunk_tx')
	assert_eq(progress[3].status, 'sending')
	assert_eq(progress[3].sent, 3)
	assert_eq(progress[3].pending_offset, 0)
	assert_eq(progress[3].pending_next, 3)
	assert_eq(progress[3].last_transfer_event, 'chunk_ack')
	assert_eq(progress[4].status, 'sending')
	assert_eq(progress[4].sent, 3)
	assert_eq(progress[4].pending_offset, 3)
	assert_eq(progress[4].pending_next, 6)
	assert_eq(progress[4].last_transfer_event, 'chunk_tx')
end
...
function tests.test_sender_retries_begin_while_waiting_ready_and_then_completes()
	...
	assert_eq(progress[2].sent, 0)
	assert_eq(progress[2].last_transfer_event, 'begin_retry_tx')
	assert_eq(progress[3].status, 'sending')
	assert_eq(progress[3].sent, 0)
	assert_eq(progress[3].pending_offset, 0)
	assert_eq(progress[3].pending_next, 3)
	assert_eq(progress[3].last_transfer_event, 'chunk_tx')
end
