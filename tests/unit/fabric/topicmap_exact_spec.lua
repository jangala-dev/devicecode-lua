...
function T.exact_topic_rules_match_only_the_declared_topic()
	local rules = topicmap.normalise_prefix_rules({
		{ topic = { 'cap', 'transfer-manager', 'main', 'rpc', 'send-blob' }, ['remote'] = { 'rpc', 'fabric', 'transfer' } },
	}, 'exact')

	local mapped1, rule1 = topicmap.map_local_to_remote(rules, { 'cap', 'transfer-manager', 'main', 'rpc', 'send-blob' })
	assert(rule1 ~= nil)
