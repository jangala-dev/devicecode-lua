...
function tests.test_restart_adoption_persists_decisions_and_marks_interrupted_jobs_failed()
	fibers.run(function ()
		local saves = {}
		local initial = { jobs = {
			staging = { job_id = 'staging', component = 'cm5', state = 'staging', phase = 'stage', stage = 'staging', created_seq = 1, updated_seq = 1 },
			active = { job_id = 'active', component = 'cm5', state = 'staging', phase = 'stage', stage = 'staging', created_seq = 4, updated_seq = 4, active_token = 'tok-stage', active_intent = { token = 'tok-stage', phase = 'stage' } },
			commit = { job_id = 'commit', component = 'cm5', state = 'awaiting_commit', phase = 'awaiting_commit', stage = 'staged', created_seq = 2, updated_seq = 2 },
			ret = { job_id = 'ret', component = 'cm5', state = 'awaiting_return', phase = 'reconcile', stage = 'awaiting_return', created_seq = 3, updated_seq = 3 },
...
function tests.test_failed_after_admission_keeps_admitted_lifecycle_marker()
	fibers.run(function ()
		local initial = { jobs = {
			j1 = { job_id = 'j1', component = 'cm5', state = 'created', phase = 'created', stage = 'created', created_seq = 1, updated_seq = 1 },
...
function tests.test_terminal_owned_artifact_is_cleaned_after_persist()
	fibers.run(function ()
		local deleted = {}
		local cleanup = {
			delete_artifact_op = function (_, artifact_ref, plan)
				deleted[#deleted + 1] = {
					artifact_ref = artifact_ref,
					state = plan.job and plan.job.state,
				}
		...
		local st, _, result = fibers.run_scope(function (scope)
			...
			local created = perform_transition(rt, {
				...
				payload = {
					...
					metadata = { artifact_cleanup = 'delete_on_terminal' },
				},
			})
			local terminal = perform_transition(rt, {
				kind = 'patch_job',
				generation = 1,
				job_id = 'j-clean',
				patch = { state = 'failed', error = 'test_failure' },
			})
			rt:cancel('test complete')
			return { created = created, terminal = terminal, deleted = deleted }
		end)
		assert_eq(st, 'ok')
		assert_eq(result.created.status, 'persisted')
		assert_eq(#result.deleted, 1)
		assert_eq(result.deleted[1].artifact_ref, 'artifact-owned')
		assert_eq(result.deleted[1].state, 'failed')
		assert_eq(result.terminal.artifact_cleanup.ok, true)
	end)
end

function tests.test_unowned_artifact_ref_is_not_cleaned_on_terminal_job()
	fibers.run(function ()
		local deleted = {}
		local cleanup = {
			delete_artifact_op = function (_, artifact_ref)
				deleted[#deleted + 1] = artifact_ref
				return op.always({ ok = true }, nil)
			end,
		}
		local st, _, result = fibers.run_scope(function (scope)
			...
			perform_transition(rt, {
				...
				payload = {
					job_id = 'j-external',
					component = 'cm5',
					artifact_ref = 'artifact-external',
				},
			})
			local terminal = perform_transition(rt, {
				kind = 'patch_job',
			...
			rt:cancel('test complete')
			return { terminal = terminal, deleted = deleted }
		end)
		assert_eq(st, 'ok')
		assert_eq(#result.deleted, 0)
		assert_eq(result.terminal.artifact_cleanup, nil)
	end)
end
...
function tests.test_begin_commit_attempt_persists_policy_and_token_before_backend_commit()
	fibers.run(function ()
		...
		local initial = { jobs = {
			j1 = {
				job_id = 'j1', component = 'cm5', state = 'committing', phase = 'commit', stage = 'committing',
...
function tests.test_restart_adoption_uses_commit_policy_for_uncertain_commits()
	fibers.run(function ()
		local initial = { jobs = {
			idem = {
				job_id = 'idem', component = 'cm5', state = 'committing', phase = 'commit', stage = 'committing',
			...
			nodup = {
				job_id = 'nodup', component = 'cm5', state = 'committing', phase = 'commit', stage = 'committing',
			...
			missing = {
				job_id = 'missing', component = 'cm5', state = 'committing', phase = 'commit', stage = 'committing',
