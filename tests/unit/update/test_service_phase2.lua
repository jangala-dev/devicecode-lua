...
function tests.test_manager_create_job_is_scoped_and_updates_status_model()
  fibers.run(function (root_scope)
    local child, caller = start_service(root_scope, { config={ schema='devicecode.update/1', components={ { id='cm5' } } } })
...
function tests.test_active_completion_releases_slot_before_later_start_admission()
  fibers.run(function (root_scope)
    ...
    local child, caller = start_service(root_scope, { config={ schema='devicecode.update/1', components={ { id='cm5' } } }, backend=active_backend })
...
function tests.test_commit_job_persists_awaiting_return_before_reconcile()
  fibers.run(function (root_scope)
    ...
    local child, caller = start_service(root_scope, {
      config={ schema='devicecode.update/1', components={ { id='cm5' } } },
...
function tests.test_restart_adoption_keeps_awaiting_commit_committable()
  fibers.run(function (root_scope)
    local initial_jobs = { jobs = { j1 = { job_id='j1', component='cm5', state='awaiting_commit', phase='awaiting_commit', stage='staged', created_seq=1, updated_seq=1 } } }
    local child, caller = start_service(root_scope, {
      config={ schema='devicecode.update/1', components={ { id='cm5' } } },
...
function tests.test_restart_adoption_starts_reconcile_for_awaiting_return()
  fibers.run(function (root_scope)
    ...
    local initial_jobs = { jobs = { j1 = { job_id='j1', component='cm5', state='awaiting_return', phase='reconcile', stage='awaiting_return', created_seq=1, updated_seq=1 } } }
    local child, caller = start_service(root_scope, {
      config={ schema='devicecode.update/1', components={ { id='cm5' } } },
      initial_jobs = initial_jobs,
      backend = {
