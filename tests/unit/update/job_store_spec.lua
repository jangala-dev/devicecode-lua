...
function T.job_store_loads_empty_namespace_and_round_trips_records()
  ...
  local ok, serr = repo:save_job({ job_id = 'j-1', state = 'available', target = 'mcu', created_at = 10 })
  assert(ok == true)
  assert(serr == nil)
  local ok2 = repo:save_job({ job_id = 'j-2', state = 'succeeded', target = 'cm5', created_at = 20 })
  assert(ok2 == true)
...
function T.job_store_deletes_job_record()
  ...
  local cap = new_control_store_cap(backing)
  local repo = job_store.open(cap, { namespace = 'update/jobs' })
  assert(repo:save_job({ job_id = 'j-1', state = 'available', target = 'mcu', created_at = 10 }) == true)
