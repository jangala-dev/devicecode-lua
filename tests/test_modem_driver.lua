...
local function test_command_manager_commands()
    test_utils.update_shim_path(shim_dir, "default_shim")
    local modem_driver = test_utils.uncached_require("services.hal.modem_driver")
    local commands = {"connect", "disconnect", "reset", "enable", "disable", "inhibit"}
    ...
    for _, cmd_name in ipairs(commands) do
        ...
        if cmd_name == 'inhibit' then assert(ret_msg == true, 'return mismatch: expected true, received '
            .. (ret_msg and "true" or "false"))
        else assert(ret_msg == cmd_name, 'return mismatch: expected '..cmd_name..', received '..ret_msg) end
    end
